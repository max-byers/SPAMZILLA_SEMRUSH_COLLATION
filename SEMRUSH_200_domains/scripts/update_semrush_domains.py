...
def process_existing_domain_files():
    """Iterate over existing domains_*.txt files and print each domain."""
    output_dir = "SEMRUSH_200_domains"
    for file in os.listdir(output_dir):
        if file.startswith("domains_") and file.endswith(".txt"):
            file_path = os.path.join(output_dir, file)
            ...
            with open(file_path, 'r') as f:
                ...
                for domain in domains:
                    print(domain)  # Replace with any other operation as needed

if __name__ == "__main__":
    try:
        process_existing_domain_files()
    except Exception as e:
