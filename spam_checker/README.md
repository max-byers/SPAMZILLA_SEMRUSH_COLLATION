...
# Spamzilla Spam Checker Configuration

## Input File Configuration

### Viewing Available Files
You can list all available CSV files in the SPAM_EXPORTS directory:

```python
from config import SpamzillaConfig

# List all available CSV files
print(SpamzillaConfig.list_available_files())
```

### Changing the Input File
You can change the input file in two ways:

1. Modify the `config.py` file directly:
```python
# In config.py
class SpamzillaConfig:
    INPUT_FILE = 'your_desired_file.csv'
```
...
# Set the input file
SpamzillaConfig.set_input_file('export-133821_2025-05-30-00-26-57.csv')
```

### Running the Script
You can run the script in multiple ways:

1. Use the configured file:
```bash
python main.py
```

2. Override with a command-line argument:
```bash
python main.py specific_file.csv
```

3. Specify an output file:
```bash
python main.py specific_file.csv output_file.csv
```

## Troubleshooting
- Ensure the file exists in the SPAM_EXPORTS directory
- The file must be a .csv
- The script will list available files if the configured file is not found 