...
def find_latest_domain_export_file():
    ...
    try:
        ...
        # Use the specific file we want to test with
        specific_file = "export-133821_2025-05-30-00-26-57.csv"
        file_path = os.path.join(domain_exports_dir, specific_file)
        
        if os.path.exists(file_path):
            log_info(f"Using specified domain export file: {specific_file}")
            return file_path
        else:
            log_error(f"Specified domain export file not found: {specific_file}")
            return None
...
def find_latest_spamzilla_file():
    ...
    try:
        # Define the SPAM_EXPORTS directory
        spam_exports_dir = os.path.join(os.path.dirname(__file__), '..', 'SPAM_EXPORTS')
        
        if not os.path.exists(spam_exports_dir):
            log_error(f"SPAM_EXPORTS directory not found: {spam_exports_dir}")
            return None
            
        # Use the specific file we want to test with
        specific_file = "Check_domains_results.2025-05-30 03_49.csv"
        file_path = os.path.join(spam_exports_dir, specific_file)
        
        if os.path.exists(file_path):
            log_info(f"Using specified export file: {specific_file}")
            return file_path
        else:
            log_error(f"Specified export file not found: {specific_file}")
            return None
            
    except Exception as e:
        log_error(f"Error finding export file: {str(e)}")
        return None

def main():
    """Parse command line arguments and run the transformation."""
    # If no input file is specified, try to find the latest Spamzilla file
    if len(sys.argv) < 2:
        input_file = find_latest_spamzilla_file()
        if not input_file:
            log_error("Please specify an input file or ensure a Spamzilla results file exists")
